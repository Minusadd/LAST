...
class MultiTaskReplayBuffer(object):
    def __init__(
        ...
        if not planning:
            ...
            self.task_buffers = dict([(idx, SimpleReplayBuffer(
                max_replay_buffer_size=max_replay_buffer_size,
                observation_dim=get_dim(self._ob_space),#FIXME 5 12
                action_dim=5, #ant-MIX 5
                #action_dim= 7, #9 #coffee
            )) for idx in tasks])
    ...
    def random_path(self, task, sequence=True):
        return self.task_buffers[task].random_path(sequence=sequence)

    def random_batch(self, task, batch_size, sequence=False, planning=False):
        #if sequence:
        if planning:
            return self.task_buffers[task].random_batch(batch_size)

        else:
            batch1, first_batch = self.task_buffers[task].random_sequence(batch_size)
            batch = self.task_buffers[task].random_batch(batch_size)
            if sequence:
                return batch1, first_batch
            else:
                #batch = self.task_buffers[task].random_batch(batch_size)
                return batch, first_batch
